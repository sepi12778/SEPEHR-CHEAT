...
local toggles = {
    ...
    HackPanel = false,
    ClickTeleport = false,
    PlayerTP = false -- Toggle state for the new feature
}
...
-- ================== NEW FEATURE: PLAYER TELEPORT PANEL ==================
local playerTPFrame = nil

local function togglePlayerTP(state)
    toggles.PlayerTP = state
    pcall(function() CoreGui.SEPEHRMODMenuV.MainFrame.Scroll["Teleport to Player"].Checkbox.Check.Visible = state end)
    ...
    if state then
        ...
        tpBtn.Size = UDim2.new(0.5, -7, 0, 35)
        tpBtn.Position = UDim2.new(0, 5, 1, -40)
        tpBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 0)
        tpBtn.Text = "Teleport"
        tpBtn.TextColor3 = Color3.new(1,1,1)
        tpBtn.Font = Enum.Font.SourceSansBold
        tpBtn.TextSize = 16
        tpBtn.Parent = playerTPFrame

        local tpCorner = Instance.new("UICorner", tpBtn)
        tpCorner.CornerRadius = UDim.new(0, 8)

        local refreshBtn = Instance.new("TextButton")
        refreshBtn.Size = UDim2.new(0.5, -7, 0, 35)
        refreshBtn.Position = UDim2.new(0.5, 2, 1, -40)
        refreshBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 200)
        refreshBtn.Text = "Refresh"
        refreshBtn.TextColor3 = Color3.new(1,1,1)
        refreshBtn.Font = Enum.Font.SourceSansBold
        refreshBtn.TextSize = 16
        refreshBtn.Parent = playerTPFrame
...
makeToggle("Teleport to Player", togglePlayerTP, 11) -- New button
makeToggle("Auto-Money (AI)", toggleAutoMoney, 12)
...
makeToggle("Hack Panel (Advanced)", toggleHackPanel, 13)
