...
local frozenPlayers = {} -- Keep track of frozen players
...
local function togglePlayerTP(state)
    ...
    if state then
        ...
        scroll.Size = UDim2.new(1, -10, 1, -135) -- Adjusted size for new buttons
        ...
        local playerRemovingConn = Players.PlayerRemoving:Connect(function(player)
            if frozenPlayers[player] then
                frozenPlayers[player] = nil -- Clean up
            end
        ...
        -- Unfreeze all players when the panel is closed
...
makeToggle("Player Menu", togglePlayerTP, 11) -- Renamed button
